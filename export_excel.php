...
$issues = fetchIssues($jiraDomain, $boardId, $sprintId, $developer, $headers, $storyPointField, $timeRange);

if (!$issues || empty($issues['issues'])) {
    die('No data found to export');
}

// Load PHPExcel or use native CSV output (for simplicity, I use CSV export here):
...
// Header row
fputcsv($output, ['Sprint Name', 'Assignee', 'Story Points', 'Resolution Date']);
...
foreach ($issues['issues'] as $issue) {
    ...
    if ($resolvedRaw) {
        try {
            ...
            $resolved = $date->format('Y-m-d');  // Format as you want
        } catch (Exception $e) {
            $resolved = $resolvedRaw; // fallback in case of error
        }
    } else {
        $resolved = '';
    }

    fputcsv($output, [$summary, $assignee, $points, $resolved]);
}
